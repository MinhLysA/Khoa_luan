...
Tổng chi phí vận hành của một cặp kho--mặt hàng trong kỳ $t$ gồm bốn thành
phần trên:
\begin{equation}
    C_t = C_{\text{hold}} + C_{\text{stock}} + C_{\text{order}} + C_{\text{over}}.
    \label{eq:tongchiphi}
\end{equation}
Bên cạnh chi phí, doanh nghiệp đặt một mục tiêu mức phục vụ: tỷ lệ đáp ứng nhu
cầu (fill rate) $\text{FR}_t^{(w)}$ trên cửa sổ trượt $w$ ngày không thấp hơn
ngưỡng $\tau$. Mục tiêu này là một \emph{ràng buộc} chứ không phải một khoản chi
phí; trong khóa luận, nó được đưa vào tín hiệu huấn luyện dưới dạng hình phạt
$P_t = \phi_{dv} \cdot \max\bigl(\tau - \text{FR}_t^{(w)},\; 0\bigr)$ (ràng buộc
mềm, Mục~\ref{subsec:fillRate}), và không được cộng vào chi phí vận hành báo cáo.
...
không lệch quá xa lý thuyết. Chênh lệch còn lại không xuất phát từ sai lệch triển khai mà phản
...
Trong Dec-POMDP chuẩn, mọi tác tử cùng theo đuổi một mục tiêu hợp tác duy
nhất. Khóa luận giữ nguyên mục tiêu hợp tác đó ở cấp toàn mạng lưới:
\begin{equation}
    R_t^{\text{team}} = \sum_{i=1}^{300} r_{i,t},
    \qquad
    \max_{\theta}\; \mathbb{E}\Bigl[\sum_{t} \gamma^{t} R_t^{\text{team}}\Bigr],
    \label{eq:mucTieuDoi}
\end{equation}
nhưng tín hiệu dùng để \emph{huấn luyện} được phân rã thành các phần thưởng cục
bộ $r_{i,t}$ của từng cặp kho--mặt hàng (Mục~\ref{subsec:rewardDecomposition}).
Nói cách khác, hệ thống có mục tiêu hợp tác ở cấp toàn mạng lưới, còn phần
thưởng cục bộ là một cách phân rã tín hiệu học nhằm giảm phương sai và giảm khó
khăn gán tín dụng. Vì các cặp trong cùng kho tương tác qua sức chứa chung, tối
ưu riêng từng $r_{i,t}$ không tự động bảo đảm tối ưu $R^{\text{team}}$; việc
phân rã này có thật sự có lợi hay không là nội dung của Câu hỏi nghiên cứu 3.
...
\begin{enumerate}
    ...
    \item \textbf{Không có nhãn hành động}. Không tồn tại nhãn hành động tối
    ưu quan sát trực tiếp cho từng ngày; chỉ quan sát được chi phí sau khi hành
    động được thực hiện. Đây là lý do không dùng học có giám sát.
...
\parencite{wolpert2001}. Wolpert và Tumer đề xuất \emph{phần thưởng chênh lệch}
(difference reward) $D_i = G(z) - G(z_{-i})$, trong đó $G(z_{-i})$ là giá trị
của hệ thống khi loại bỏ đóng góp của tác tử $i$ --- một đại lượng phản thực
(counterfactual) phải được tính thêm.

Khóa luận \emph{không} cài đặt phần thưởng chênh lệch theo đúng nghĩa này. Thay
vào đó, khóa luận dùng \emph{phần thưởng phân rã cục bộ} (local factored
reward): mỗi tác tử nhận $r_{i,t} = -(C_{i,t} + P_{i,t})$, gồm chi phí vận hành
$C_{i,t}$ và hình phạt mức phục vụ $P_{i,t}$ của chính cặp kho--mặt hàng mà nó
quản lý, với $\sum_i r_{i,t} = R_t^{\text{team}}$ (công thức~\ref{eq:mucTieuDoi}).
Cách phân rã này dựa trên cùng động cơ giảm khó khăn gán tín dụng như phân tích
của Wolpert và Tumer. Nó khả thi ở đây vì phần lớn chi phí của một cặp (lưu kho,
thiếu hàng, đặt hàng) do chính quyết định của cặp đó gây ra; riêng phần tràn kho
--- đại lượng thực sự ghép nối giữa các cặp --- được phân bổ theo tỷ trọng hàng
mới về của từng cặp (Mục~\ref{subsec:sucChua}).
...
Nghiên cứu của Ding và cộng sự \cite{ding2022cdppo} (CD-PPO) phát biểu chính xác bài toán tồn kho đơn cửa hàng nhiều SKU dưới dạng tài nguyên kho dùng chung làm ghép nối các tác tử vốn độc lập. Họ so sánh trực tiếp IPPO và MAPPO có chia sẻ tham số (parameter sharing), báo cáo hiệu quả cao hơn các phương pháp đối chứng khi quản lý tài nguyên kho chung.
...
tăng đáng kể. Cần lưu ý rằng biến thể đối chứng của họ sử dụng trạng thái
