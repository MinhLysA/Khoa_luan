...
và không tối ưu hóa đồng thời ràng buộc sức chứa dùng chung, phạt tràn    và mức phục vụ --- những đặc trưng quan trọng trong bài toán đa kho -- đa mặt
...
\begin{itemize}
    \item \textbf{Mức phục vụ theo chu kỳ ($\alpha$ -- Cycle Service Level / Type-1):} Là xác suất không xảy ra hiện tượng thiếu hàng trong một chu kỳ bổ sung tồn kho, tức $P(D_L \le s)$ trong đó $D_L$ là nhu cầu tích lũy trong thời gian giao hàng $L$ và $s$ là điểm đặt hàng lại. Tỷ lệ critical ratio trong mô hình Newsvendor (\ref{eq:newsvendor}) phản ánh chính xác phân vị $z_\alpha$ của thước đo $\alpha$.
    \item \textbf{Tỷ lệ đáp ứng nhu cầu ($\beta$ -- Fill Rate / Type-2):} Là tỷ lệ phần trăm tổng nhu cầu của khách hàng được đáp ứng ngay lập tức từ lượng hàng sẵn có trong kho.
    \item \textbf{Tỷ lệ sẵn sàng ($\gamma$ -- Ready Rate):} Là tỷ lệ thời gian mà mức tồn kho khả dụng duy trì giá trị dương.
\end{itemize}
...
số. Lợi ích:
\begin{itemize}
    \item Số tham số cần học không tăng theo số tác tử (ở đây: $500$).
...
Bảng~\ref{tab:soSanhCongTrinh} tóm tắt so sánh tổng quan giữa khóa luận này và các công trình nghiên cứu liên quan tiêu biểu.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{
        ...
        \textbf{Khóa luận này} & \textbf{10 kho song song} & \textbf{50} & \textbf{Có} & \textbf{M5 Walmart} & \textbf{IPPO} \\
        \hline
    \end{tabularx}
\end{table}

Thay vì đưa ra các khẳng định tuyệt đối về tính duy nhất hay tính "đầu tiên", khóa luận định vị đóng góp theo hướng \textbf{tái lập và mở rộng có kiểm soát} đối với ứng dụng MARL trong kiểm soát tồn kho, tập trung vào ba trọng tâm chính:
\begin{enumerate}
    \item Khảo sát sâu cấu hình \textbf{10 kho song song thuần túy (non-multi-echelon)} với ràng buộc sức chứa kho dùng chung ở quy mô $500$ cặp kho--mặt hàng, tạo sự đối sánh rõ rệt với các mô hình đa cấp (multi-echelon) của MABIM \parencite{yang2023mabim} hay Liu và cộng sự \parencite{liu2025happo}.
    \item Đánh giá \textbf{ablation study tường minh} tác động của tín hiệu thưởng phân rã cục bộ (local reward decomposition) kèm phạt ràng buộc fill rate cửa sổ trượt ở quy mô 500 tác tử, làm rõ cơ chế gán tín dụng và tốc độ hội tụ của IPPO.
    \item Hiệu chỉnh môi trường mô phỏng lost-sales trực tiếp từ dữ liệu bán lẻ thực tế M5 Walmart và thực hiện kiểm chứng đối chứng nghiêm ngặt với các chính sách cổ điển EOQ, $(s,S)$ và Newsvendor.
\end{enumerate}