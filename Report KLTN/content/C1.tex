...
trong cuộc thi dự báo M5 do Makridakis và cộng sự \cite{makridakis2022background} tổ chức, ghi
...
2016\footnote{Các thống kê được tính từ hai tệp \path{calendar.csv} và
\path{sales_train_evaluation.csv} trong bộ dữ liệu M5 \parencite{m5dataset}.}.
Tổ hợp cửa hàng và mặt hàng tạo thành $30.490$ chuỗi thời gian nhu cầu ở cấp
độ chi tiết nhất.
...
$68\%$. Áp dụng tiêu chí phân loại của Syntetos và cộng sự \cite{syntetos2005categorization} với
các ngưỡng khoảng cách giữa hai lần phát sinh nhu cầu $p = 1{,}32$ và bình
phương hệ số biến thiên $CV^2 = 0{,}49$ (trong đó Kostenko và Hyndman \cite{kostenko2006}
đã chứng minh kết quả biên chính xác với giá trị giới hạn $p = 4/3 \approx 1{,}333$ và đề xuất quy tắc phân loại chỉnh sửa), có tới $95{,}2\%$ số chuỗi thuộc hai
...
\begin{table}[htbp]
    ...
    bộ dữ liệu M5 theo tiêu chí của Syntetos và cộng sự \cite{syntetos2005categorization}}
    \label{tab:phanloainhucau}
    \begin{tabular}{|l|c|c|}
        \hline
        \textbf{Nhóm} & \textbf{Đặc điểm} & \textbf{Tỷ lệ} \\
        \hline
        Smooth       & $p < 1{,}32$; $CV^2 < 0{,}49$   & $3{,}2\%$  \\ \hline
        Erratic      & $p < 1{,}32$; $CV^2 \ge 0{,}49$ & $1{,}6\%$  \\ \hline
        Intermittent & $p \ge 1{,}32$; $CV^2 < 0{,}49$ & $75{,}7\%$ \\ \hline
        Lumpy        & $p \ge 1{,}32$; $CV^2 \ge 0{,}49$ & $19{,}5\%$ \\
        \hline
    \end{tabular}
\end{table}

Cần nói rõ rằng các thống kê trên được tính trên toàn bộ $30.490$ chuỗi của bộ
dữ liệu gốc. Tập thực nghiệm của khóa luận là một tập con đã qua sàng lọc: các
mặt hàng mà cửa hàng trên thực tế không kinh doanh (nhu cầu trung bình dưới
$0{,}2$ đơn vị mỗi ngày ở phần lớn cửa hàng) bị loại bỏ, vì chúng không tạo
thành bài toán đặt hàng lại mà chỉ thêm nhiễu vào tín hiệu học. Sau sàng lọc,
tập $300$ cặp dùng trong thực nghiệm vẫn giữ tỷ lệ ngày không phát sinh giao
dịch ở mức $57{,}4\%$ --- nghĩa là tính gián đoạn được bảo toàn --- với nhu
cầu trung bình $3{,}24$ và trung vị $0{,}74$ đơn vị mỗi ngày. Chi tiết quy
trình sàng lọc được trình bày ở Mục~\ref{subsec:duLieu}.
...
Thứ nhất, tính bất định kép của nhu cầu và thời gian giao hàng. Nhu
...
Thứ hai, bài toán đánh đổi giữa nhiều nhóm chi phí xung đột. Mục tiêu
...
\begin{table}[htbp]
    \centering
    \caption{Bốn thành phần chi phí vận hành và mục tiêu mức phục vụ trong bài
    toán tồn kho đang xét}
    \label{tab:chiphi}
    \begin{tabular}{llp{6.2cm}}
        \toprule
        \textbf{Thành phần} & \textbf{Ký hiệu} & \textbf{Chiều tác động} \\
        \midrule
        ...
        Phạt tràn kho            & $p_{tk}$    & Tăng khi hàng về vượt sức chứa kho \\
        \midrule
        Mục tiêu mức phục vụ     & $\tau = 85\%$ & Ràng buộc nghiệp vụ, không phải chi phí; khi huấn luyện được đưa vào dưới dạng hình phạt với hệ số $\phi_{dv}$ \\
        \bottomrule
    \end{tabular}
\end{table}

Thứ ba, ràng buộc sức chứa tạo ra sự phụ thuộc giữa các mặt hàng. Các
...
Thứ tư, quy mô không gian hành động. Với $N$ kho và $M$ mặt hàng, mỗi
cặp có $K$ mức đặt hàng khả dĩ, không gian hành động chung là tích Descartes
có kích thước $K^{NM}$. Với cấu hình của khóa luận ($N = 10$, $M = 30$,
$K = 6$), con số này là $6^{300}$ --- vượt xa khả năng xử lý của bất kỳ phương
...
thể. Mô hình lượng đặt hàng kinh tế của Harris \cite{harris1913} giả định nhu cầu
xác định và đều, không có tình trạng thiếu hàng. Chính sách $(s, S)$ được
Scarf \cite{scarf1960} chứng minh tối ưu cho hệ đơn kho với nhu cầu độc lập giữa
các kỳ, chi phí đặt hàng cố định và cơ chế giữ đơn hàng chờ. Mô hình người bán
báo của Arrow và cộng sự \cite{arrow1951} chỉ tối ưu cho một chu kỳ đơn lẻ, không xét tới
...
hàng khi thiếu hàng --- vốn phù hợp với bối cảnh bán lẻ --- Zipkin \cite{zipkin2008}
...
quy mô nhỏ: Oroojlooyjadid và cộng sự \cite{oroojlooyjadid2022} áp dụng mạng Q sâu cho trò chơi phân
phối bia, còn Gijsbrechts và cộng sự \cite{gijsbrechts2022} đánh giá học tăng cường sâu trên ba
...
bộ theo hướng tiếp cận học độc lập của Tan \cite{tan1993}, trong khi toàn bộ
...
vài mặt hàng, trên dữ liệu nhu cầu sinh theo phân phối lý thuyết. Ngay cả những
công trình gần đây nhất cùng bối cảnh cũng để lại những khoảng trống rõ rệt.
Zhu và Wu \cite{zhu2026spatiotemporal} cũng sử dụng bộ dữ liệu M5 và học tăng
cường đa tác tử, nhưng giả định nguồn cung không giới hạn và không đưa sức chứa
kho vào hàm mục tiêu, nên sự ràng buộc giữa các mặt hàng trong cùng một kho
không được mô hình hóa. Ở chiều ngược lại, khung làm việc của Nam và cộng sự
\cite{jeong2026alca} có xét ràng buộc sức chứa nhưng chỉ trên một kho duy nhất
và trên dữ liệu tổng hợp, đồng thời hàm phần thưởng không quy về đơn vị tiền tệ
nên khó diễn giải về mặt kinh tế.

Khoảng trống mà khóa luận hướng tới nằm ở giao của ba yếu tố: mở rộng tới quy
mô hàng trăm cặp kho--mặt hàng bằng cơ chế chia sẻ tham số; mô hình hóa ràng
buộc sức chứa hữu hạn dùng chung giữa các mặt hàng, tính riêng cho từng kho
theo quy mô nhu cầu không đồng nhất của kho đó; và đánh giá so sánh với các
chính sách cổ điển ở cùng một mức phục vụ, trên môi trường mô phỏng hiệu chỉnh
từ dữ liệu bán lẻ thực tế.
...
\begin{itemize}
    ...
    nhiên, thời gian giao hàng biến động, ràng buộc sức chứa, bốn thành phần
    chi phí vận hành (lưu kho, thiếu hàng, đặt hàng, tràn kho) và mục tiêu mức
    phục vụ; nhu cầu lấy từ bộ dữ liệu M5 Walmart.

    \item Thiết kế tín hiệu phần thưởng phân rã cục bộ (local factored reward)
    cho từng cặp kho--mặt hàng, kèm thành phần phạt mức phục vụ tính trên cửa
    sổ trượt, nhằm giảm khó khăn của bài toán gán tín dụng giữa các tác tử
    \parencite{wolpert2001}.
    ...
    $300$ cặp kho--mặt hàng, sử dụng ước lượng lợi thế tổng quát
...
\begin{enumerate}
    \item Câu hỏi 1: Trong cùng điều kiện mức phục vụ --- tức khi mọi
    chính sách đều phải đạt tỷ lệ đáp ứng nhu cầu tối thiểu $85\%$ --- PS-IPPO
    (IPPO với chia sẻ tham số) có đạt tổng chi phí vận hành thấp hơn các chính
    sách tồn kho cổ điển (EOQ, $(s, S)$, Newsvendor) hay không, và khác biệt đó
    có ý nghĩa thống kê hay không?

    \item Câu hỏi 2: Cơ chế chia sẻ tham số có duy trì được hiệu năng
    trên các cặp kho--mặt hàng có quy mô nhu cầu khác nhau hay không; và, như
    một câu hỏi mở rộng, chính sách học được có áp dụng được cho những mặt hàng
    không xuất hiện trong huấn luyện hay không?

    \item Câu hỏi 3: Thiết kế tín hiệu phần thưởng --- phần thưởng phân rã
    cục bộ theo từng cặp so với phần thưởng toàn cục của cả mạng lưới, và chuẩn
    hóa phần thưởng theo từng cặp so với không chuẩn hóa --- ảnh hưởng như thế
    nào đến độ ổn định huấn luyện và hiệu năng của PS-IPPO?
\end{enumerate}

Câu hỏi 1 là câu hỏi chính. Mức $85\%$ là mục tiêu nghiệp vụ được đặt trước
khi thực nghiệm; phép so sánh ở một mức phục vụ cao hơn (bằng đúng mức IPPO đạt
được) chỉ được dùng như phân tích bổ sung.
...
\begin{itemize}
    \item Cấu hình hệ thống: $N = 10$ kho, $M = 30$ mặt hàng, tương
    ứng $300$ cặp ra quyết định; độ dài mỗi tập huấn luyện là $365$ ngày; thời
    gian giao hàng ngẫu nhiên trong khoảng từ một đến ba ngày.

    \item Cấu trúc mạng lưới: các kho hoạt động song song và độc lập
    ...
    \item Nguồn dữ liệu: bộ dữ liệu bán lẻ M5 Walmart
    \cite{makridakis2022background}, cụ thể là hai tệp
    \path{sales_train_evaluation.csv} chứa $30.490$ chuỗi doanh số theo
    ngày trên $1.941$ ngày, và \path{calendar.csv} chứa thông tin lịch, ngày
    ...
    \item Phương pháp đối chứng: ba chính sách tồn kho cổ điển EOQ
    \cite{harris1913}, $(s, S)$ \cite{scarf1960} và Newsvendor
    \cite{arrow1951}, với tham số được tối ưu bằng tìm kiếm lưới trên
    chính môi trường mô phỏng nhằm bảo đảm tính công bằng của so sánh.

    \item Chỉ số đánh giá: tổng chi phí vận hành, tỷ lệ đáp ứng nhu
    cầu, và cơ cấu các thành phần chi phí lưu kho, thiếu hàng, đặt hàng, tràn
    kho. Việc so sánh chi phí được thực hiện bổ sung ở cùng một mức
    phục vụ nhằm tránh kết luận sai lệch khi hai chính sách nằm ở hai điểm
    khác nhau trên đường đánh đổi chi phí --- mức phục vụ.
\end{itemize}
...
\begin{itemize}
    ...
    kho theo mô hình của Clark và Scarf \cite{clark1960}, cùng các hiện tượng đi kèm như
...
Khóa luận kết hợp ba nhóm phương pháp. Về lý thuyết, bài toán được mô
...
\parencite{puterman1994, sutton2018}. Về thực nghiệm, nhóm xây dựng
...
M5, huấn luyện tác tử và thu thập số liệu vận hành. Về phân tích, mọi
so sánh giữa các phương pháp đều được thực hiện trên cùng một tập hạt giống
đánh giá, báo cáo giá trị trung bình kèm độ lệch chuẩn, và được xác nhận bằng
kiểm định thống kê phù hợp trước khi kết luận --- theo khuyến nghị về tính tái
lập trong đánh giá học tăng cường sâu của Henderson và cộng sự \cite{henderson2018} và
Agarwal và cộng sự \cite{agarwal2021}.

\section{Ý nghĩa khoa học và thực tiễn của đề tài}
\label{sec:ynghia}

Về mặt khoa học, khóa luận đóng góp một khung mô hình hóa bài toán tồn
...
Về mặt thực tiễn, kết quả của khóa luận cung cấp bằng chứng định lượng
...
\begin{itemize}
    \item Chương 1 --- Tổng quan về đề tài: trình bày bối cảnh, tính
    cấp thiết, mục tiêu, câu hỏi nghiên cứu, phạm vi và phương pháp nghiên cứu.

    \item Chương 2 --- Cơ sở lý thuyết và các công trình liên quan:
    ...
    \item Chương 3 --- Phân tích và thiết kế mô hình: chi tiết hóa mô
    ...
    \item Chương 4 --- Thực nghiệm và đánh giá kết quả: trình bày cấu
    hình thực nghiệm, kết quả so sánh với các chính sách cổ điển, các khảo sát
    loại trừ và phân tích độ nhạy.

    \item Chương 5 --- Kết luận và hướng phát triển: tổng kết kết quả
